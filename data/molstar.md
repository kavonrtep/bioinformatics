...
-  Try scrolling the mouse do to the structure (when the mouse in the structure
window pane)?
...
- Click on part of the structure in the 3D view. What happens in the sequence panel? Click on the the same resudue again. See what happens.
...
- Hide/show ions - what ion are present in this structure?
...
which are the three-dots in a row.
...
- Explore the coloring by hydrophobicity on a membrane protein structure, such as Human cannabioid receptor CB2  - PDB accesssion -`5ZTY`
- Can you identify the transmembrane regions based on the hydrophobicity coloring?


### Viewing amino acid side chains

- Find PDB `1DXZ` entry, which is M2 Transmembrane Segment of the Nicotinic Acetylcholine Receptor Alpha-Subunit.
- Which aiminos acids are ~1 turn away from Met 243?
...
- When clicking on “M”, all of the side chains within 5 Å will appear. Identify the AA which are one turn away from Met 243.
...
Go the RCSB PDB (https://www.rcsb.org) and search for the PDB ID `2Y5T`.
...
- In default ribbon representation, can you do not see the actual surface of the channel, the space is empty but in reallity it is filled side chains of amino acids. To see the actual surface, we need to add Molecular Surface representation.
...
- The side arm of D23 is interaction to which amino acid? and from which chain?
...
- In the left control panem use Download Structure option to load PDB structure `4HHB`
- Next, use a Download Structure option again to load Alphafold model `AF-P69905
- What is the difference between the two structures? 
- Set colorin of Polymer to Model Index to distinguish between the two structures.
...
- To align the two structures:
   ...
   - Click "Superpose"
- Check the alignment statistics in the botom log panel - What is the RMSD between the two structures?
...
- The leucine zipper is the dimerization domain: two alpha-helices from different chains pack together, with leucines residues forming a hydrophobic stripe along each helix. 
- We will visualize the hydrophobic core of this coiled-coil by highlighting Leu and Val residues.
---

 - Loading 1DGC in Mol* - 
   - Go to [molstar.org](https://molstar.org/)
   - In the left control panem use Download Structure option to load PDB structure `1DGC`
...
- Identify Leu and Val residues in both chains:
  ...
  - Identify and select Ile, Leu and Val residues in both chains. For selecting you can use the sequence panel or click directly on residues in the 3D view.
...
- When your satisfied with the view, export the image with Screenshot option
